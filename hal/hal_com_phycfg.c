...
char	rtl8188_file_path[PATH_LENGTH_MAX];
...
)
{
	...
	{
		rtw_merge_string(rtl8188_file_path, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName);

		if (rtw_is_file_readable(rtl8188_file_path) == _TRUE)
		{
			rlen = rtw_retrive_from_file(rtl8188_file_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
			if (rlen > 0)
			{
				rtStatus = _SUCCESS;
